...
class LSTM_double(nn.Module):
    ...
    def forward(self, X):
        ...
        sample_size = X.size()[1]  # sample_size = batch_size
        '''do this LSTM forward'''
        c1_0 = torch.zeros(sample_size, n_hidden)  # 第一层记忆状态初值，设置为全0
        h1_0 = torch.zeros(sample_size, n_hidden)  # 第一层隐藏状态初值，设置为全0
        c2_0 = torch.zeros(sample_size, n_hidden)     # 第二层记忆状态初值，设置为全0
        h2_0 = torch.zeros(sample_size, n_hidden)     # 第二层隐藏状态初值，设置为全0
        c1 = c1_0    # 第一层记忆状态初始化
        h1 = h1_0    # 第一层隐藏状态初始化
        c2 = c2_0    # 第二层记忆状态初始化
        h2 = h2_0    # 第二层隐藏状态初始化
